...
10     continue
    ...
    !At this point we need to compute B and v
    call build_BM
    call build_v
    ...
    !!This inital s vector probably needs to be updated to s = pmatvec2II(r)+transpose(B)*v 
    s = pmatvec2II(r)  
    p = s
    ...
    resNE = 0
    call cpu_time(ce)
    write(*,"(A,G12.5,A)") "  STARTING JOINT INNER ITERATIONS AT: ",ce-cs," seconds" ! Increase witdth in format descriptor. - OFGN 1/10/23
    do i=1,max_initer
       ...
       !compute u = B*p where be is the cross grad matrix (computed prior to inner iterations)
       !note B is sparse so we'll have to write a function to do this Bmatvec
       !*u = Bmatvec(B,p) 
       ...
       !This needs to be updated with s = pmatvec2II(r) + transpose(B)*v
       s = pmatvec2II(r)
