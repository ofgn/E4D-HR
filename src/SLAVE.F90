...
!!$         deallocate(dist)
    ...
    subroutine receive_sigma
        ...
        if (.not. allocated(sigma_re)) allocate (sigma_re(model_size))
    ...
    subroutine receive_sigmai
        ...
        implicit none
        integer :: i
        if (.not. allocated(sigma_im)) allocate (sigma_im(model_size))
    ...
    subroutine weight_jaco
        ...
        do i = 1, data_assignments(my_rank, 2) - data_assignments(my_rank, 1) + 1
            row = data_assignments(my_rank, 1) + i - 1
    ...
    subroutine compute_pmatvec2
       ...
       !!Jacobian transpose, then return the results
        ...
        ind1 = data_assignments(my_rank, 1)
        ind2 = data_assignments(my_rank, 2)
       ...
       !!do M'x
        ...
        call MPI_REDUCE(sol2, sol2, n_elements, MPI_DOUBLE, MPI_SUM, 0, COMM, ierr)

        return

       !!Return the solution to master
        call MPI_SEND(sol2, n_elements, MPI_DOUBLE, 0, tag, COMM, ierr)
    ...
    subroutine compute_pmatvec2_dbl
       ...
       !!Jacobian transpose, then return the results
        ...
        real*8, dimension(n_elements) ::sol2d
        integer :: COMM
       ...
       !!do M'x
        ...
        call MPI_REDUCE(sol2d, sol2d, n_elements, MPI_DOUBLE_PRECISION, MPI_SUM, 0, COMM, ierr)
    ...
    subroutine start_inv
        ...
        allocate (X1(n_elements), sol1(n_data_assigned), X2(nm), sol2(n_elements))
    ...
    subroutine start_invi
        implicit none
        allocate (X1i(n_elements), sol1i(n_data_assigned), X2i(nm), sol2i(n_elements))
    ...
    subroutine send_Jcol
       ...
       !!Jacobian transpose, then return the results
        ...
        do i = 1, n_data_assigned
            sol1(i) = Jaco(i, col)
        end do

        call MPI_GATHERV(sol1, n_data_assigned, MPI_DOUBLE, dummy, recvcnts, displs, MPI_DOUBLE, 0, COMM, ierr)
        return
    ...
    subroutine compute_pmatvec1
       ...
       !!Jacobian transpose, then return the results
        ...
        !allocate(x(n))
        call MPI_BCAST(X1, n_elements, MPI_DOUBLE, 0, COMM, ierr)
       ...
       !!Return the solution to master
        call MPI_SEND(sol1, n_data_assigned, MPI_DOUBLE, 0, tag, E4D_COMM, ierr)
    ...
    subroutine compute_pmatvec1_dbl
       ...
       !!Jacobian transpose, then return the results
        ...
        real*8, dimension(n_elements) :: X1d
        integer :: COMM
        ...
        call MPI_BCAST(X1d, n_elements, MPI_DOUBLE_PRECISION, 0, COMM, ierr)
       ...
       !!compute Mx
        ...
        call MPI_GATHERV(sol1, n_data_assigned, MPI_DOUBLE, dummy, idummy, idummy, MPI_DOUBLE, 0, COMM, ierr)

        return

       !!Return the solution to master
        call MPI_SEND(sol1, n_data_assigned, MPI_DOUBLE, 0, tag, COMM, ierr)
    ...
    subroutine compute_pmatvec2i
       ...
       !!do M'x
        ...
        call MPI_REDUCE(sol2i, sol2i, n_elements, MPI_DOUBLE_PRECISION, MPI_SUM, 0, E4D_COMM, ierr)
        return

       !!Return the solution to master
        call MPI_SEND(sol2, n_elements, MPI_DOUBLE, 0, tag, E4D_COMM, ierr)
    ...
    subroutine compute_pmatvec1i
       ...
       !!Jacobian transpose, then return the results
        ...
        !call MPI_BCAST(n,1,MPI_INTEGER,0,E4D_COMM,ierr)
        !allocate(x(n))
        call MPI_BCAST(X1i, n_elements, MPI_DOUBLE_PRECISION, 0, E4D_COMM, ierr)
      ...
      call MPI_GATHERV(sol1i, n_data_assigned, MPI_DOUBLE_PRECISION, dummy, idummy, idummy, MPI_DOUBLE_PRECISION, 0, E4D_COMM, ierr)
        return
    ...
    subroutine get_J_on_off
        ...
        if (.not. allocated(J_on_off)) allocate (J_on_off(n_elements))
        J_on_off = .true.
    ...
    subroutine send_sens
        ...
        njaco = data_assignments(my_rank, 2) - data_assignments(my_rank, 1) + 1
        allocate (sens(n_elements))
        sens = 0
        tag = 0
        do i = 1, n_elements
        ...
        call MPI_SEND(sens, n_elements, MPI_DOUBLE, 0, tag, COMM, ierr)
    ...
    subroutine send_full_jaco
        ...
        integer :: njaco, i, tag, ierr
        njaco = data_assignments(my_rank, 2) - data_assignments(my_rank, 1) + 1
        call MPI_SEND(Jaco, njaco*n_elements, MPI_DOUBLE, 0, my_rank, E4D_COMM, ierr)
