...
!__________________________________________________________________________________________________
   subroutine build_tetgen4(i_flag)
    ...
    !!tetgen (for the body)
      ...
      logical :: i_flag
      integer :: m, d1, d2, d3, el1, el2, el3, el4, el5, i, j, k, tcount, nedge, ecount, bv, ios, nsp2, ii, mesh_prefix_length, fstat
      ...
      mesh_prefix = cfg_file(1:mesh_prefix_length)

    !!OPEN AND READ MESH FILE
      inquire (file=trim(cfg_file), exist=file_exists)
      if (.not. file_exists) call check_minp(1, -1)

      open (10, file=cfg_file, status='old', action='read')
...
!!!!!!!!!!!!!!!!!!NOW WE ARE READY TO CONSTRUCT THE TETGEN INPUT FILE!!!!!!!!!!!!!!!!!!!!!!
    ...
    !!Change the extension of mesh_file to .poly
      ...
      open (11, file=cfg_file, status='replace', action='write')
...
11    continue
      ...
      write (*, *) "DONE BUILDING ", trim(cfg_file)

1001  continue

    !!Delete previous mesh files if they exist
      inquire (file=cfg_file(1:npre)//'1.node', exist=file_exists)
      if (file_exists) call system('rm '//cfg_file(1:npre)//'1.node')
      inquire (file=cfg_file(1:npre)//'1.ele', exist=file_exists)
      if (file_exists) call system('rm '//cfg_file(1:npre)//'1.ele')
      inquire (file=cfg_file(1:npre)//'1.face', exist=file_exists)
      if (file_exists) call system('rm '//cfg_file(1:npre)//'1.face')
      inquire (file=cfg_file(1:npre)//'1.neigh', exist=file_exists)
      if (file_exists) call system('rm '//cfg_file(1:npre)//'1.neigh')

    !!CALL TETGEN TO BUILD THE MESH
      write (*, *) "    CALLING TETGEN         "
      if (nz > 0) then
         write (*, *) trim(tetcom)//" -pnq"//trim(qual)//"a"//trim(max_vol)//"aAA"//" "//trim(cfg_file)
         call system(trim(tetcom)//" -pnq"//trim(qual)//"a"//trim(max_vol)//"aAA"//"         "//trim(cfg_file))
      else
         write (*, *) trim(tetcom)//" -pnq"//trim(qual)//"a"//trim(max_vol)//"AA"//" "//trim(cfg_file)
         call system(trim(tetcom)//" -pnq"//trim(qual)//"a"//trim(max_vol)//"AA"//"         "//trim(cfg_file))
      end if
    ...
    !!make sure the boundary flags are correct and rebuild if not
      ...
      call check_nodes(cfg_file)
      !end if

      !! Build the conductivity file if necessary
      if (nz > 0) then
         call build_sig(cfg_file, npre, nz, z_sig, z_isig, zon_labs, i_flag)
      end if
      ...
      if (vtk_flag .eq. 1) then
         ...
         if (file_exists) then
            ...
            if (file_exists) then
               inquire (file=trim(mesh_prefix)//".sig", exist=file_exists)
               if (file_exists) then
                  call convert_to_vtk(trim(mesh_prefix), nz - 1)
               else
                  call mesh_error_log(106, "Cannot find "//trim(mesh_prefix)//".sig")
               end if
            else
...
!___________________________________________________________________________________________________
   subroutine check_minp(indx, ios)
      ...
      case (1)
         open (52, file="mesh_build.log", action='write', status='old', position='append')
         write (52, *) "Cannot find the mesh configuration file: ", trim(cfg_file)
         close (52)
      ...
      case (2)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) "There was a problem reading the quality factor max element volume in ", trim(cfg_file)
            close (52)
            write (*, *) "There was a problem reading the quality factor max element volume in ", trim(cfg_file)
            call crash_exit
         end if
      ...
      case (3)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) "There was a problem reading the minimum elevation in ", trim(cfg_file)
            close (52)
            write (*, *) "There was a problem reading the minimum elevation in ", trim(cfg_file)
            call crash_exit
         end if
      ...
      case (4)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) "There was a problem reading the build tetgen flag ", trim(cfg_file)
            close (52)
            write (*, *) "There was a problem reading the build tetgen flag ", trim(cfg_file)
            call crash_exit
         end if
      ...
      case (5)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) "There was a problem reading the tetgen executable name in ", trim(cfg_file)
            close (52)
            write (*, *) "There was a problem reading the tetgen executable name in ", trim(cfg_file)
            call crash_exit
         end if
      ...
      case (6)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) "There was a problem reading the triangle executable name in ", trim(cfg_file)
            close (52)
            write (*, *) "There was a problem reading the triangle executable name in ", trim(cfg_file)
            call crash_exit
         end if
      ...
      case (7)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) "There was a problem reading the number of control points ", trim(cfg_file)
            close (52)
            write (*, *) "There was a problem reading the number of control points ", trim(cfg_file)
            call crash_exit
         end if
      ...
      case (10)
         ...
         write (52, *) 'SEE POINT ', ios, ' IN MESH CONFIGURATION FILE ', trim(cfg_file)
         close (52)
         write (*, *) 'SURFACE POINTS (flag 1 or 2) MUST BE DEFINED BEFORE INTERNAL POINTS (flag 0)'
         write (*, *) 'SEE POINT ', ios, ' IN MESH CONFIGURATION FILE ', trim(cfg_file)
         call crash_exit
      ...
      case (12)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) 'There was a problem reading the number of piecewise linear complexes in', trim(cfg_file)
            close (52)
            write (*, *) 'There was a problem reading the number of piecewise linear complexes in', trim(cfg_file)
            call crash_exit
         end if
      ...
      case (14)
         ...
         write (52, *) 'for piecewise linear complex: ', ios, ' in ', trim(cfg_file)
         close (52)
         write (*, *) 'There was a problem reading the number of points and boundary number'
         write (*, *) 'for piecewise linear complex: ', ios, ' in ', trim(cfg_file)
         call crash_exit
      ...
      case (16)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) 'There was a problem reading the number of holes', trim(cfg_file)
            close (52)
            write (*, *) 'There was a problem reading the number of holes', trim(cfg_file)
            call crash_exit
         end if
      ...
      case (19)
         if (ios .ne. 0) then
            open (52, file="mesh_build.log", action='write', status='old', position='append')
            write (52, *) 'There was a problem reading the number of zones in ', trim(cfg_file)
            close (52)
            write (*, *) 'There was a problem reading the number of zones in ', trim(cfg_file)
            call crash_exit
         end if
...
10       continue
      ...
      case (27)
         ...
         write (*, *)
         write (*, *) 'ERROR'
         write (*, *) 'There are only ', ios, ' boundary points specified in: ', trim(cfg_file)
         ...
         write (52, *)
         write (52, *) 'ERROR'
         write (52, *) 'There are only ', ios, ' boundary points specified in: ', trim(cfg_file)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   subroutine build_sig(cfg_file, npre, nzon, zsigs, zsigsi, zlabs, ifg)
      implicit none
      character*40 :: cfg_file
      integer :: npre, nzon
      ...
      write (*, *) "BUILDING "//cfg_file(1:npre)//"sig"
      open (11, file=cfg_file(1:npre)//"1.ele", status='old', action='read')
      open (12, file=cfg_file(1:npre)//"sig", status='replace', action='write')
