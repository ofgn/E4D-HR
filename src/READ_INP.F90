...
contains
    ...
    subroutine read_input
        ...
        read (10, *, IOSTAT=io_status) smode; call check_inp(101, junk)
        ...
        if (trim(adjustl(smode)) == 'analytic' .or. trim(adjustl(smode)) == 'ertanalytic') then
            ...
            case DEFAULT
                read (smode, *, IOSTAT=io_status) mode; call check_inp(1, junk)
            end select
        end if
    ...
    !! if running in mode zero attempt the read the input files, report, and exit
        ...
        read (10, *, IOSTAT=io_status) cfg_filename
        ...
        if (mode > 1 .and. mode .ne. 21 .and. mode .ne. 31 .and. mode .ne. 41) then
            read (10, *, IOSTAT=io_status) efile; call check_inp(3, junk)
            read (10, *, IOSTAT=io_status) sig_filename; call check_inp(4, junk)
        ...
        if (mode == 3 .or. mode == 4 &
            ...
            read (10, *, IOSTAT=io_status) invfile; call check_inp(6, junk)
            read (10, *, IOSTAT=io_status) refmod_file; call check_inp(7, junk)

        elseif (mode == 23 .or. mode == 43 .or. mode == 24 .or. mode == 44) then

            read (10, *, IOSTAT=io_status) invfile, iinvfile; call check_inp(6, junk)
            read (10, *, IOSTAT=io_status) refmod_file; call check_inp(7, junk)
    ...
    !!read the time lapse data list file
        if (tl_ly) then
            ...
            read (10, *, IOSTAT=io_status) tl_file, check; call check_inp(8, check)
            ...
            if (rt_flag) then
                ...
                open (21, file=trim(tl_file), status='old', action='read', IOSTAT=io_status)
                read (21, *, IOSTAT=io_status) ntl; call check_inp(9, junk)
                allocate (tl_dfils(ntl), tlt(ntl))

                do i = 1, ntl
                    read (21, *, IOSTAT=io_status) tl_dfils(i), tlt(i); call check_inp(10, i)
    ...
    !!Read the inversion options file and check for errors
        ...
        nrz = 0
        read (10, *, IOSTAT=io_status) nrz; call check_inv_opts(1, junk)
       ...
       !!in the inverse options file
            ...
            EXTRNL = .false.
            read (10, *, IOSTAT=io_status) str1; call check_inv_opts(2, i)
            if (trim(str1) .ne. 'EXTERNAL' .and. trim(str1) .ne. 'External' .and. trim(str1) .ne. 'external') then
                read (str1, *, IOSTAT=io_status) izn
                call check_inv_opts(2, i)
            else
                EXTRNL = .true.
            end if

            read (10, *, IOSTAT=io_status) izn; call check_inv_opts(3, i)
            read (10, *, IOSTAT=io_status) izn; call check_inv_opts(4, i)

            if (EXTRNL) then
                read (10, *, IOSTAT=io_status) str1; call check_inv_opts(33, i)
            else
                read (10, *, IOSTAT=io_status) itmp(i); call check_inv_opts(5, i)
                if (itmp(i) > max_nlink) then
                    max_nlink = itmp(i); 
                end if
            end if
            read (10, *, IOSTAT=io_status) str1; call check_inv_opts(6, i)
            read (10, *, IOSTAT=io_status) rdum; call check_inv_opts(7, i)
        end do
        ...
        read (10, *, IOSTAT=io_status) nrz; call check_inv_opts(8, junk)
        smetric = 0
        C_targ = 0
        ...
        do i = 1, nrz
          ...
          !!from any zone that exists in the mesh
            ...
            elseif (str1 == "pref" .or. str1 == "PREF" .or. str1 == "Pref") then
                ...
                read (str1, *, IOSTAT=io_status) C_targ(i); call check_inv_opts(14, i)
            ...
            read (10, *, IOSTAT=io_status) zwts(i, 1); call check_inv_opts(15, i)
    ...
    !!check for reference model
        ...
        do i = 1, nrz
            if (smetric(i, 3) == 1) then
                ...
                if (allocated(element_map)) then
                    k = 0
                    read (11, *, IOSTAT=io_status) nstmp
                    if (invi) then

                        do j = 1, nstmp
                            read (11, *, IOSTAT=io_status) rsig, isig
                    ...
                    if (invi) then
                        ...
                        do j = 1, nstmp
                            read (11, *, IOSTAT=io_status) refsig(j)
                            if (io_status .ne. 0) then
                                call check_inv_opts(20, j)
                            end if
                        end do
        ...
        read (10, *, IOSTAT=io_status) beta, del_obj, beta_red; call check_inv_opts(21, junk)
        beta_s = beta
        conv_opt = 1

        read (10, *, IOSTAT=io_status) norm_chi2; call check_inv_opts(22, junk)
        read (10, *, IOSTAT=io_status) min_initer, max_initer; call check_inv_opts(23, junk)
        delta_initer = 0.001
        initer_conv = 1e-4; 
        read (10, *, IOSTAT=io_status) min_sig, max_sig; call check_inv_opts(24, junk)

        read (10, *, IOSTAT=io_status) up_opt; call check_inv_opts(25, junk)
        if (up_opt == 1) then
            read (10, *, IOSTAT=io_status) nlsp; call check_inv_opts(26, junk)
            nlsp = nlsp + 1
            allocate (lsp(nlsp))
        ...
        read (10, *, IOSTAT=io_status) cull_flag, cull_dev; call check_inv_opts(28, junk)
        allocate (Wd_cull(nm))
        Wd_cull = 1.0
        nec = 0
...
103     continue
    ...
    subroutine read_survey
        ...
        read (10, *, IOSTAT=io_status) ne; call check_inp(12, junk)

        allocate (e_pos(ne, 4))
        do i = 1, ne
            read (10, *, IOSTAT=io_status) junk, etmp; call check_inp(13, i)
        ...
        ! Read in the survey
        read (10, fmt=*, IOSTAT=io_status) nm
        ...
        if (i_flag) then
            ...
            do i = 1, nm
                read (10, *, IOSTAT=io_status) junk, s_conf(i, 1:4), dobs(i), Wd(i), dobsi(i), Wdi(i); call check_inp(18, i)
            ...
            do i = 1, nm
                read (10, *, IOSTAT=io_status) junk, s_conf(i, 1:4), dobs(i), Wd(i); call check_inp(18, i)
    ...
    subroutine translate_electrodes
        ...
        read (21, *, IOSTAT=io_status) xorig, yorig, zorig; call check_inp(16, junk)
        close (21)
    ...
    subroutine read_conductivity
        ...
        if (mode .ne. 1) then
            ...
            if (.not. file_exists) then
                read (sig_filename, *, IOSTAT=io_status) tsig
                if (io_status .ne. 0) then
            ...
            read (10, *, IOSTAT=io_status) model_size; call check_inp(23, 0)
            ...
            if (i_flag) then
                ...
                do i = 1, model_size
                    read (10, *, IOSTAT=io_status) sigma_re(i), sigma_im(i); call check_inp(23, i)
                    if (sigma_re(i) <= 0 .or. sigma_im(i) <= 0) call check_inp(26, i)
                end do
            else
                do i = 1, model_size
                    read (10, *, IOSTAT=io_status) sigma_re(i); call check_inp(23, i)
