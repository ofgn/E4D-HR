...
40  continue
  ...
  subroutine get_survey
    ...
    read(10,*,IOSTAT=io_status) ne
    if(io_status .ne. 0) goto 1007
    write(51,*) "NUMBER OF ELECTRODE LOCATIONS = ",ne
    
    
    allocate(e_pos(ne,4))
    do i=1,ne
       read(10,*,IOSTAT=io_status) junk,etmp
       if(io_status .ne. 0) goto 1008
       if(junk>ne) then
    ...
    if(mode > 1) then
       ...
       read(21,*,IOSTAT=io_status) xorig,yorig,zorig
       if(io_status .ne. 0) goto 1009
       close(21)
    ...
    !!Read in the survey
    open(51,file='e4d.log',status='old',action='write',position='append')
    read(10,*,IOSTAT=io_status) nm,n_add,ntot,ropt
    if(io_status .ne. 0) goto 1010
    ...
    if(i_flag) then
       ...
       do i=1,nm
          read(10,*,IOSTAT=io_status) junk,s_conf_all(i,1:4),dobs(i),Wd_all(i),dobsi(i),Wd_alli(i),mgrp(i)
          if(io_status .ne. 0) goto 1011
       ...
       do i=1,nm
          read(10,*,IOSTAT=io_status) junk,s_conf_all(i,1:4),dobs(i),Wd_all(i),mgrp(i)
          if(io_status .ne. 0) goto 1011
          if(Wd_all(i) <= 0) then
             write(*,*) "NEGATIVE OR ZERO STD DEVIATION AT DATUM ",i
             write(*,*) "SETTING DEVIATION TO VERY LARGE "
   ...
   read(10,*,IOSTAT=io_status) nR_inds
   if(io_status .ne.0) goto 1012
   ...
   do i=1,nR_inds
      read(10,*,IOSTAT=io_status) junk,R_pts(i,1),R_pts(i,2),R_pts(i,3)
      if(io_status .ne. 0) goto 1014
   end do
   close(10)
