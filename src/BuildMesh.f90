            ...
            do i = 1, nplc
                ...
                do m = 1, nc_plc(i) - 1
                    if (tflag(temp_int(m)) > 0 .and. tflag(temp_int(m + 1)) > 0 .and. &
                        tflag(temp_int(m)) < 3 .and. tflag(temp_int(m + 1)) < 3) then
                        splc_count = splc_count + 1
                        write (*, *) nedge + splc_count, temp_int(m), temp_int(m + 1), 3 + i
...
!____________________________________________________________________________
        ...
        do i = 1, nsp2
            ...
            sp2(i, 3) = 1/(1/w1 + 1/w2 + 1/w3) &
                        *(dble(all_points2(d1, 3))/w1 &
                          + dble(all_points2(d2, 3))/w2 + dble(all_points2(d3, 3))/w3)
10          continue
        end do
      ...
      !!Define the non boundary PLC's if necessary
        if (nplc > 0) then
            ...
            do i = 1, nplc
                ...
                write(format, '(a, i0, a)') '(I8,', nc_plc(i), 'I8)'
                write (11, format) nc_plc(i), temp_int(edge_seq) ! Add format specifier to force single line output with the Intel Fortran Compiler. - OFGN 27/4/23
