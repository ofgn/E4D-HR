...
#include "petsc/finclude/petscksp.h"
        ...
        call KSPSetType(KS, KSPCG, perr)          ! Use Conjugate Gradient solver
        call KSPGetPC(KS, P, perr)
        call PCSetType(P, PCGAMG, perr)           ! Use AMG preconditioner
        call PCSetFromOptions(P, perr)

        ! Original GMRES solver
        ! call KSPCreate(PETSC_COMM_SELF,KS,perr)
        ! call KSPSetOperators(KS,A,A,perr)
        ! call KSPGetPC(KS,P,perr)
...
1001    format(" NOTICE: ELECTRODE ", I5, " IS ", E12.5, " DISTANCE UNITS FROM THE NEAREST NODE")  ! Increase witdth in format descriptor. - OFGN 1/10/23
    ...
    subroutine compute_source_currents
        ...
        logical, dimension(n_elements) :: check_ele
