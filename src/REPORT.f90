...
contains
    ...
    !> @brief Log an error message to a file and to stdout
    ...
    subroutine init_log
        ...
        open (51, file='e4d.log', status='replace', action='write', IOSTAT=io_stat)
    ...
    subroutine nreport(tag)
        ...
        if (tag == 3) then
            ...
            write (67, "(A12,I8,A19,I8,A9)") " Estimating ", npar, " parameters out of ", n_elements, " elements"
            close (67)
        end if
        ...
        if (tag == 20) then
            open (67, file='e4d.log', status='old', action='write', position='append')
            write (67, *) "CANNOT FIND OUTPUT FILE: ", trim(out_file)
        ...
        if (tag == 57) then
            ...
            write (67, *) " Number of elements in ", trim(cfg_filename(1:npre))//".ele is: ", n_elements
            write (67, *) " Number of conductivity values in ", trim(sig_filename), " is: ", model_size
            write (67, *) " Aborting ..."
            close (67)

            write (*, *)
            write (*, *) " The number of conductivity values is not equal to the number of mesh elements"
            write (*, *) " Number of elements in ", trim(cfg_filename(1:npre))//".ele is: ", n_elements
            write (*, *) " Number of conductivity values in ", trim(sig_filename), " is: ", model_size
            write (*, *) " Aborting ..."
        ...
        if (tag == 62) then
            open (67, file='e4d.log', status='old', action='write', position='append')
            write (67, "(A,I10.10)") "  Number of elements:               ", n_elements
        ...
        if (tag == 64) then
            ...
            do i = 1, n_rank - 1
                ...
                if (data_assignments(i, 2) - data_assignments(i, 1) + 1 > i2) i2 = data_assignments(i, 2) - data_assignments(i, 1) + 1
            end do
            open (67, file='e4d.log', status='old', action='write', position='append')
            write (67, *)
        ...
        if (tag == 69) then
            open (67, file='e4d.log', status='old', action='write', position='append')
            write (67, *)
            write (67, *) " WRITING SIMULATED SURVEY FILE: ", trim(sig_filename), ".srv"
            close (67)
            write (*, *) " WRITING SIMULATED SURVEY FILE: ", trim(sig_filename), ".srv"
        end if
