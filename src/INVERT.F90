...
contains
    ...
    subroutine pcgls
        ...
        real, dimension(n_elements) :: s, p, xpar
        ...
        if (wopt) then
            call cpu_time(ce)
            write (*, "(A,G12.5,A)") "  SENDING DATA NOISE TO SLAVES AT: ", ce - cs, " seconds"
            call send_data_noise
        end if

            !!Build the model part of b
        call cpu_time(ce)
        write (*, "(A,G12.5,A)") "  CONSTRUCTING CONSTRAINT RESIDUALS AT: ", ce - cs, " seconds"
        do i = 1, ccount
            ...
            case (1)
                if (invi) then
               b(nm + i) = Wm(i)*(sigma_re(rblock(i, 1)) - sigma_im(rblock(i, 1)) - sigma_re(rblock(i, 2)) + sigma_im(rblock(i, 2)))
                else
                    b(nm + i) = Wm(i)*(sigma_re(rblock(i, 2)) - sigma_re(rblock(i, 1)))
                end if
            case (2)
                if (invi) then
               b(nm + i) = Wm(i)*(sigma_re(rblock(i, 1)) - sigma_im(rblock(i, 1)) - sigma_re(rblock(i, 2)) + sigma_im(rblock(i, 2)))
            ...
            case (3)
                ...
                case (0)
                    if (invi) then
                        b(nm + i) = Wm(i)*(C_targ(rbi) + sigma_re(rblock(i, 1)) - sigma_im(rblock(i, 1)))
                    else
                        b(nm + i) = Wm(i)*(C_targ(rbi) - sigma_re(rblock(i, 1)))
                    end if
                case (1)
                    if (invi) then
                        b(nm + i) = Wm(i)*(refsig(rblock(i, 1)) + sigma_re(rblock(i, 1)) - sigma_im(rblock(i, 1)))
                    else
                        b(nm + i) = Wm(i)*(refsig(rblock(i, 1)) - sigma_re(rblock(i, 1)))
                    end if
                case (2)
                    if (invi) then
                        b(nm + i) = Wm(i)*(prefsig(rblock(i, 1)) + sigma_re(rblock(i, 1)) - sigma_im(rblock(i, 1)))
                    else
                        b(nm + i) = Wm(i)*(prefsig(rblock(i, 1)) - sigma_re(rblock(i, 1)))
                    end if
                end select
            ...
            case (4)
                ...
                case (1)
                    if (invi) then
                        b(nm + i) = Wm(i)*(refsig(rblock(i, 1)) - sigma_im(rblock(i, 1)))
                    else
                        b(nm + i) = Wm(i)*(refsig(rblock(i, 1)) - sigma_re(rblock(i, 1)))
                    end if
                case (2)
                    if (invi) then
                        b(nm + i) = Wm(i)*(prefsig(rblock(i, 1)) - sigma_im(rblock(i, 1)))
                    else
                        b(nm + i) = Wm(i)*(prefsig(rblock(i, 1)) - sigma_re(rblock(i, 1)))
                    end if
                end select
            ...
            case (5)
                if (invi) then
                    b(nm + i) = Wm(i)*(sigma_im(rblock(i, 2)) - sigma_im(rblock(i, 1)))
                else
                    b(nm + i) = Wm(i)*(sigma_re(rblock(i, 2)) - sigma_re(rblock(i, 1)))
                end if
            ...
            case (7)
                ...
                case (0)
                    if (invi) then
                        b(nm + i) = Wm(i)*((C_targ(rbi) - sigma_im(rblock(i, 2))) - (C_targ(rbi) - sigma_im(rblock(i, 1))))
                    else
                        b(nm + i) = Wm(i)*((C_targ(rbi) - sigma_re(rblock(i, 2))) - (C_targ(rbi) - sigma_re(rblock(i, 1))))
                    end if
                case (1)
                    if (invi) then
                        b(nm + i) = Wm(i)*((sigma_im(rblock(i, 2)) - refsig(rblock(i, 2))) - &
                                           (sigma_im(rblock(i, 1)) - refsig(rblock(i, 1))))
                    else
                        b(nm + i) = Wm(i)*((sigma_re(rblock(i, 2)) - refsig(rblock(i, 2))) - &
                                           (sigma_re(rblock(i, 1)) - refsig(rblock(i, 1))))
                    end if
                case (2)
                    if (invi) then
                        b(nm + i) = Wm(i)*((sigma_im(rblock(i, 2)) - prefsig(rblock(i, 2))) - &
                                           (sigma_im(rblock(i, 1)) - prefsig(rblock(i, 1))))
                    else
                        b(nm + i) = Wm(i)*((sigma_re(rblock(i, 2)) - prefsig(rblock(i, 2))) - &
                                           (sigma_re(rblock(i, 1)) - prefsig(rblock(i, 1))))
                    end if
                end select
            ...
            case (8)
                ...
                case (1)
                    if (invi) then
                        b(nm + i) = Wm(i)*(sigma_re(rblock(i, 1)) - sigma_im(rblock(i, 1)) - &
                                      sigma_re(rblock(i, 2)) + sigma_im(rblock(i, 2)) + refsig(rblock(i, 2)) - refsig(rblock(i, 1)))
                    else
                        b(nm + i) = Wm(i)*((sigma_re(rblock(i, 2)) - refsig(rblock(i, 2))) - &
                                           (sigma_re(rblock(i, 1)) - refsig(rblock(i, 1))))
                    end if
                case (2)
                    if (invi) then
                        b(nm + i) = Wm(i)*(sigma_re(rblock(i, 1)) - sigma_im(rblock(i, 1)) - &
                                    sigma_re(rblock(i, 2)) + sigma_im(rblock(i, 2)) + prefsig(rblock(i, 2)) - prefsig(rblock(i, 1)))
                    else
                        b(nm + i) = Wm(i)*((sigma_re(rblock(i, 2)) - prefsig(rblock(i, 2))) - &
                                           (sigma_re(rblock(i, 1)) - prefsig(rblock(i, 1))))
                    end if
            ...
            case (12)
                b(nm + i) = -Wm(i)*cg_wts(rblock(i, 2), 1)*sigma_re(rblock(i, 1))
                do j = 1, 4
                    nbr = neighbors(rblock(i, 1), j)
                    if (nbr > 0) then
                        b(nm + i) = b(nm + i) - Wm(i)*cg_wts(rblock(i, 2), j + 1)*sigma_re(nbr)
                    end if
                end do
...
10          continue
        ...
        resNE = 0
        call cpu_time(ce)
        write (*, "(A,G12.5,A)") "  STARTING INNER ITERATIONS AT: ", ce - cs, " seconds"
        do i = 1, max_initer
...
100     continue
    ...
    function pmatvec1II(x)
        ...
        implicit none
        real, dimension(n_elements) :: x
        real, dimension(nm + ccount) :: pmatvec1II
        integer :: i, rbi, nbr, j, k

        pmatvec1II = 0

        call do_pmatvec1(n_elements, nm, pmatvec1II(1:nm), x(1:n_elements))
...
10          continue
    ...
    function pmatvec2II(r)
        ...
        real, dimension(n_elements) :: pmatvec2II

        !!do the full matrix (Jacobian) part of the multiplication in parallel
        pmatvec2II = 0
        call do_pmatvec2(n_elements, nm, pmatvec2II(1:n_elements), r(1:nm))
...
10          continue
    ...
    subroutine pcglsi
        ...
        s = pmatvec2i(nm, n_elements, ncon, ccount, wrows, wcols, Wm, r(1:nm + ncon), beta, npar, par_map, nec, ex_cols, ex_vals)
        ...
        do i = 1, max_initer
            ...
            q = pmatvec1i(nm, n_elements, ncon, ccount, wrows, wcols, Wm, p, beta, npar, par_map, nec, ex_cols, ex_vals)
            ...
            s = pmatvec2i(nm, n_elements, ncon, ccount, wrows, wcols, Wm, r, beta, npar, par_map, nec, ex_cols, ex_vals)
