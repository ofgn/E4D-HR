...
\subsection{Installing E4D} \label{install}
To be completed.
...
The phase \( \phi_c \) associated with \( \sigma^* \) is defined as:

\begin{equation}
\sigma^* = |\sigma^*| e^{i\phi_c},
\end{equation}

Here, \( \phi_c \) represents the phase lag of the induced alternating current (AC) electric field behind the injected AC current. The real part \( \sigma' \) is linked to the movement of charges through the material, while the imaginary part \( \sigma'' \) is associated with charge storage mechanisms, such as polarisation effects.
...
\begin{description}
    ...
    \item[\texttt{initial\_model}] (char)\hfill \\
    \begin{itemize}
        \item \texttt{initial\_model = `none'}: Set to none unless the user requires inversion constraints based on a reference model.
        \item \texttt{initial\_model = `sig\_file\_name'}: Alternatively, a \texttt{sig\_filename} can be specified.
    \end{itemize}
\end{description}
...
\begin{verbatim}
    ...
    initial_beta reduction_criteria beta_reduction
    chi2_target
    min_iter max_iter
    min_sig max_sig
    beta_mode
...
\begin{description}
    ...
    \item[\texttt{reduction\_criteria}] (real)\hfill \\
     The minimum fractional decrease in the objective function between outer iterations that may occur before beta is reduced. A conservative value for is \texttt{reduction\_criteria = 0.05}. Larger values will generally decrease time to convergence, but may also provide solutions that violate the model constraints more than what is necessary to appropriately fit the data.

     \item[\texttt{beta\_reduction}] (real)\hfill \\
     The beta reduction factor. If the fractional decrease in the objective function between outer iterations is less than or equal to \texttt{reduction\_criteria}, then beta is reduced by a factor of \texttt{beta\_reduction} for the next iteration. A conservative value is \texttt{beta\_reduction = 0.5}.
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=5.0in]{figures/two_blocks_dcr_ip.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=5.0in]{figures/inversion_dcr_ip.png}}
